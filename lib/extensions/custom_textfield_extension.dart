  ...
  List<TextInputFormatter> get _formatters {
    switch (this) {
      case InputType.email:
        return [FilteringTextInputFormatter.deny(RegExp(r'\s')), FilteringTextInputFormatter.allow(RegExp(r'[a-zA-Z0-9@._-]'))];
      case InputType.phone:
        return [FilteringTextInputFormatter.digitsOnly, LengthLimitingTextInputFormatter(20)];
      ...
      case InputType.panNumber:
        return [FilteringTextInputFormatter.deny(RegExp(r'\s')), LengthLimitingTextInputFormatter(10), PanCardInputFormatter()];
      case InputType.aadhaarNumber:
        return [FilteringTextInputFormatter.deny(RegExp(r'\s')), LengthLimitingTextInputFormatter(12)];
      default:
        return [];
    }
  }
...
extension TextFormFieldExtensions on TextFormField {
  TextFormField withDecoration({InputDecoration? decoration}) {
    return TextFormField(decoration: decoration);
  }

  TextFormField withFormatters({required List<TextInputFormatter> formatters}) {
    return TextFormField(inputFormatters: formatters);
  }

  TextFormField withKeyboardType({required TextInputType keyboardType}) {
