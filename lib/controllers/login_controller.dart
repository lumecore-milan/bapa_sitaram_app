...
class LoginController extends GetxController {
  ...
  Future<(bool, String)> sendOtp({required String mobileNo}) async {
    ...
    try {
      await _apiInstance.post(url: APIConstant().apiLogin, requestBody: {'mobile_no': mobileNo}, isFormData: false).then((data) {
        if (data.isNotEmpty) {
          otpSent.value = true;
          otpSent.refresh();
        }
  ...
  Future<(bool, String)> register({required String mobileNumber}) async {
    ...
    try {
      ...
      Map<String, dynamic> body = {'mobile_no': mobileNumber, 'profile_photo': '', 'name': name.text, 'email': email.text, 'pan_card': panCard.text, 'address': address.text, 'pin_code': pinCode.text};
