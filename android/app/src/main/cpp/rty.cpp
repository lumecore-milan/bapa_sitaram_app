...
Java_com_bapasitaram_bagdana_MainActivity_getApiBaseUrl(JNIEnv* env, jclass clazz) {
    if (!g_baseUrl.empty()){
        return env->NewStringUTF(g_baseUrl.c_str());
    }else {
...
extern "C" JNIEXPORT jstring JNICALL
Java_com_bapasitaram_bagdana_MainActivity_getStoredBaseUrl(JNIEnv* env, jclass) {
    if (g_baseUrl.empty()) {
